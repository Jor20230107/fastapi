from src.model.user import PublicUser, PrivateUser, SignInUser
from src.data.__init__ import (conn, curs, get_db, IntegrityError)
...
# 유저 조회는 is_public에 따라 PublicUser 또는 PrivateUser 를 반환한다.
def get_one(name: str, is_public: bool = True) -> PublicUser | PrivateUser:
    qry = "select * from where name=:name"
    params = {"name": name}
